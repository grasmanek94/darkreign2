    ...
    if (Terrain::mapInit && TerrainData::ScreenToTerrain(::Input::MousePos().x, ::Input::MousePos().y, pos) && WorldCtrl::MetreOnMap(pos.x, pos.z))
    {
      switch (units)
      {
        case 0x6EC59045: // "Grains"
        {
          ...
          Utils::Sprintf(buf, 128, (const CH*)L"G%4d,%4d Q%d", gx, gz, q);
          break;
        }

        case 0xFAD28AB8: // "Cells"
          Utils::Sprintf(buf, 128, (const CH*)L"%4d,%4d", WorldCtrl::MetresToCellX(pos.x), WorldCtrl::MetresToCellZ(pos.z));
          break;

        case 0x6ECAAA17: // "Clusters"
          Utils::Sprintf(buf, 128, (const CH*)L"%4d,%4d", WorldCtrl::MetresToClusterX(pos.x), WorldCtrl::MetresToClusterZ(pos.z));
          break;

        case 0x47ED7A3C: // "Metres"
        default:
          Utils::Sprintf(buf, 128, (const CH*)L"%6.1fm,%6.1fm", pos.x, pos.z);
          break;
      }
    }

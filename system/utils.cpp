...
namespace Utils
{
  ...
  {
    ASSERT(str1 && str2);
    return (wcsicmp((const wchar_t *)str1, (const wchar_t *)str2));
  }
  ...
  {
    ASSERT(str1 && str2);
    return (wcsnicmp((const wchar_t *)str1, (const wchar_t *)str2, count));
  }
  ...
  {
    ASSERT(dest && src);
    return (CH*)(wcscpy(( wchar_t *)dest, (const wchar_t *)src));
  }
  ...
  CH * Strncpy(CH *dest, const CH *src, U32 count)
  {
    ASSERT(dest && src);
    ASSERT(count >= 0);
    return (CH*)(wcsncpy(( wchar_t *)dest, (const wchar_t *)src, count));  
  }
  ...
  {
    ASSERT(dest && src);
    return (CH*)(wcscat(( wchar_t *)dest, (const wchar_t *)src));
  }
  ...
  {
    ASSERT(str);
    return (CH*)(wcsrchr((const wchar_t *)str, c));
  }
  ...
  {
    ASSERT(str && substr);
    return (CH*)(wcsstr((const wchar_t *)str, (const wchar_t *)substr));
  }
  ...
  S32 CDECL Sprintf(CH *buffer, S32 bufSize, const CH *format, ...)
  {
    ...
    // Store the number of characters written
    va_start(args, format);
    count = vswprintf((wchar_t *)buffer, (const wchar_t *)format, args);
    va_end(args);
  ...
  int Ansi2Unicode(CH *str, U32 size, const char *ansi)
  {
    ...
    int i = MultiByteToWideChar(GetACP(), MB_PRECOMPOSED, ansi, ansilen, (wchar_t *)str, size);
    str[i++] = CH('\0');
    return (i);
  }
  ...
  S32 CharDecode64(U8 *dst, const char *src, U32 srcSize, U32 dstSize)
  {
    ...
    {
      base64data.i = 0;
	  int i = remainder, shift = 18;
      for (; i; i--, srcSize--, shift -= 6)
      {

  ...
  Bool Server::ProcessPacket(User &user, const Packet &packet)
  {
    ...
    switch (packet.GetCommand())
    {
      ...
      case ClientMessage::UserLogin:
      {
        ...
        if (packet.GetData((const ClientMessage::Data::UserLogin*&)data))
        {
      ...
      case ClientMessage::UserMigrating:
      {
        ...
        if (packet.GetData((const ClientMessage::Data::UserMigrating*&)data))
        {
          ...
          if (session)
          {
            ...
            {
              LDIAG("User " << user.GetName().str << " migrated")
      ...
      case ClientMessage::UserReconnect:
      {
        ...
        if (packet.GetData((const ClientMessage::Data::UserReconnect*&)data))
        {
          ...
          for (NList<User>::Iterator u(&disconnected); *u; ++u)
          {
            ...
            {
              LDIAG("User " << (*u)->GetName().str << " is attempting reconnection")
      ...
      case ClientMessage::SessionCreate:
      {
        ...
        if (packet.GetData((const ClientMessage::Data::SessionCreate*&)data))
        {
          LDIAG("User " << user.GetName().str << " wants to create a session " << data->name.str << " max " << data->maxUsers)
          ...
          {
            LDIAG("Session " << data->name.str << " already exists")
          ...
          {
            LDIAG("Session " << data->name.str << " created")
      ...
      case ClientMessage::SessionConnect:
      {
        ...
        if (packet.GetData((const ClientMessage::Data::SessionCreate*&)data))
        {
          LDIAG("User " << user.GetName().str << " wants to connect to session " << data->name.str << " max " << data->maxUsers)
          ...
          if (session)
          {
            ...
            if (session->password == data->password)
            {
              ...
              {
                LDIAG("User " << user.GetName().str << " connected")
      ...
      case ClientMessage::SessionDestroy:
      {
        ...
        if (packet.GetData((const ClientMessage::Data::SessionDestroy*&)data))
        {
          LDIAG("User " << user.GetName().str << " wants to destroy session " << data->name)
      ...
      case ClientMessage::SessionJoin:
      {
        ...
        if (packet.GetData((const ClientMessage::Data::SessionJoin*&)data))
        {
          LDIAG("User " << user.GetName().str << " wants to join to session " << data->name)
          ...
          if (session)
          {
            ...
            if (session->password == data->password)
            {
              ...
              {
                LDIAG("User " << user.GetName().str << " joined")
      ...
      case ClientMessage::SessionLock:
        if (user.session)
        {
          ...
          {
            LDIAG("Locking session " << user.session->name.str)
      ...
      case ClientMessage::SessionUnlock:
        if (user.session)
        {
          ...
          if (user.session->host == &user)
          {
            LDIAG("Unlocking session " << user.session->name.str)
            user.session->flags &= ~SessionFlags::Locked;
            user.session->SendInfo();
          }
          else
      ...
      case ClientMessage::SessionKick:
        ...
        if (user.session)
        {
          ...
          if (user.session->host == &user)
          {
            ...
            if (packet.GetData((const ClientMessage::Data::SessionKick*&)data))
            {

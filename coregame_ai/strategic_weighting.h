  ...
  namespace Weighting
  {
    ...
    template <class DATA> struct Cuts
    {
      ...
      struct Item
      {
        ...
        // Constructor
        Item(typename NBinTree<DATA, F32>::Node typename DATA::* node)
        : aboveCut(node),
          belowCut(node)
        {
        }
      ...
      // The node
      typename NBinTree<DATA, F32>::Node typename DATA::* node;

      // Constructor
      Cuts(typename NBinTree<DATA, F32>::Node typename DATA::* node)
      : items(&Item::node),
        node(node)
      {
      }
    ...
    public:
      ...
      void Process(Cuts<DATA> &cuts) const
      {
        ...
        for (typename NBinTree<DATA, KEY>::Iterator i(&active); *i; i++)
        {
          ...
          Cuts<DATA>::Item *ii = cuts.items.Find(priority);
          if (!ii)
          {

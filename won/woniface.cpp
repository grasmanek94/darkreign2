...
namespace WonIface
{
	...
	using server_address = std::wstring;
	using routing_server_client = std::wstring;
	using won_identity = std::wstring;
	///////////////////////////////////////////////////////////////////////////////
	//
	// Constants
	//
	...
	server_address address;
		...
		// Constructor
		ChatServer(const CH *name, const server_address &address, U32 numPlayers, Bool password, Bool permanent)
		: name(name), 
			address(address),
	...
	routing_server_client routingServer;
		...
		(
			const CH *roomName, 
			const CH *password, 
			const routing_server_client& routingServer
		)
		: roomName(roomName),
			password(password),
	...
	// Our WON identity
	static won_identity identity;

	// Routing server we are currently dealing with
	static routing_server_client currentRoutingServer = NULL;
	...
	static void CreateAccountCallback(const won_identity::AuthResult &result, AbortableContext *);
	static void LoginAccountCallback(const won_identity::AuthResult &result, AbortableContext *);
	static void ChangePasswordCallback(const won_identity::AuthResult &result, AbortableContext *);
	...
	void CreateAccount(const char *username, const char *password)
	{
		...
		// Build an Identity
		identity = won_identity(username, community, password, "", authServers.servers, authServers.num);
	...
	Bool CheckStoredKey()
	{
...
#endif
		{
			...
			won_identity::SetCDKey(key);
			won_identity::SetLoginKeyFile(loginKey);
			if (!won_identity::LoadVerifierKeyFromFile(publicKey))
			{
	...
	// CreateAccountCallback
	//
	void CreateAccountCallback(const won_identity::AuthResult &result, AbortableContext *context)
	{
	...
	// LoginAccountCallback
	//
	void LoginAccountCallback(const won_identity::AuthResult &result, AbortableContext *context)
	{
		ASSERT(context)
	...
	// ChangePasswordCallback
	//
	void ChangePasswordCallback(const won_identity::AuthResult &result, AbortableContext *context)
	{
...
*/
	...
	// Update a game
	void UpdateGame(const GameName &name, U32 size, const U8 *data)
	{
	...
	void IgnorePlayer(const PlayerName &player)
	{
	...
	void UnignorePlayer(const PlayerName &player)
	{
